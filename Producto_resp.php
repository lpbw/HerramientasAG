...
class Producto extends UploadFiles{
    ...
    public function get($id=NULL){
        ...
        $consulta  = "SELECT nombre,id,id_catalogo_productos,precio,
                costo, id_proveedor, modificado, archivo_ficha_tecnica, 
    ...
    public function update($nombre,$id,$id_catalogo_productos,
        $precio,$costo,$id_proveedor,$modificado,$archivo_ficha_tecnica,
        $imagen,$descripcion, $codigo_familia, $unidad_metrica, 
        $exportar_microsip=0, $stock="", $codigo_descuento="", 
        $tipo_moneda_usa="",$unidad_metrica_ingles="", $recargo=0,
        $esProductoEspecial = FALSE, $peso, $stock_fabrica, $tienda,$descripcion_l, $stock_proveedor, $factor, $flete_cliente, $flete_proveedor, $v_cliente, $v_proveedor){
        
        $nombre = strtoupper($nombre);
        $descripcion = strtoupper($descripcion);
        if($imagen!=""){
            $imagen = "imagen = '".
                    $this->resizeImageAndUpload('imagenProducto_'.$this->id,'archivos','imagen',400)."',";
            $this->resizeImageAndUpload('ch_imagenProducto_'.$this->id,'archivos','imagen',92);
        } else 
            $imagen = "";
        
        if($archivo_ficha_tecnica!=""){
            $archivo_ficha_tecnica = "archivo_ficha_tecnica = '".
                    $this->uploadFile('FichaTecnica_'.$this->id,'archivos','archivo_ficha_tecnica')."',";
        } else 
        ...
        $consulta = "UPDATE Productos SET 
                nombre = '$nombre',
                id_catalogo_productos = '$id_catalogo_productos',
                $costo_precio
                id_proveedor = '$id_proveedor',
                modificado = '$modificado',
                $archivo_ficha_tecnica
                $imagen
                $tipo_moneda_usa_query
                descripcion = '$descripcion',
                codigo_familia = '$codigo_familia',
                unidad_metrica = '$unidad_metrica',
                exportar_microsip = '$exportar_microsip',
                stock = '$stock',
                codigo_descuento = '$codigo_descuento',
                unidad_metrica_ingles = '$unidad_metrica_ingles',
				recargo=$recargo,
				peso=$peso,
				stock_fabrica=$stock_fabrica,
				tienda=$tienda,
				descripcion_l='$descripcion_l',
				stock_proveedor=$stock_proveedor, 
				factor='$factor',
				flete_cliente='$flete_cliente',
				flete_proveedor='$flete_proveedor',
				v_cliente='$v_cliente',
				v_proveedor='$v_proveedor'
            WHERE id=".$this->id;

        $resultado = mysql_query($consulta) or print("No se ha podido actualizar $consulta ".  mysql_error());
		
        $this->get($this->id);
        if(!($esProductoEspecial==TRUE)){//se compara con false porque en algunos casos se pasa un string
    ...
    public function create($nombre,$id_catalogo_productos,$precio,$costo,$id_proveedor,$modificado,
        ...
        $resultado = mysql_query($consulta) or print("Creacion no exitosa $consulta".  mysql_error());
        ...
        if($resultado){
            ...
            if($imagen!=""){
                $imagen = "imagen = '".
                        $this->resizeImageAndUpload('imagenProducto_'.$id,'archivos','imagen',400)."'";
            ...
            if($archivo_ficha_tecnica!="" ){
                $consulta = "UPDATE Productos
                    SET  $archivo_ficha_tecnica
                    WHERE id = ".  $id;
                $resultado = mysql_query($consulta) or print("Actualizacion de ficha tecnica no exitosa $consulta".  mysql_error());
            }
			 if( $imagen !=""){
                $consulta = "UPDATE Productos
                    SET $imagen 
    ...
    public function agregarACarrito(){
        ...
        else $_SESSION['carrito'] = array($this);
    }
    
    public function updateOneAttr($attrName, $attrValue, $idUsuario, 
            $requireRevision = NULL, $id_cotizacion = '', $id_version_cotizacion = ''){
        
        $query = "UPDATE Productos SET $attrName = '$attrValue'
            WHERE id=$this->id";
        ...
        if($result){
            if($requireRevision && $idUsuario){
                ...
                $this->insertNewRequestRevision($attrName, $idUsuario, 
                        $attrValue, $actualValueToChange,
    ...
    private function insertNewRequestRevision( $attrName, $id_usuario, $proposedValue, $actualValue,
            $id_cotizacion, $id_version_cotizacion ){
        ...
        $query = "
            INSERT INTO revisionCambios_Productos( id_producto, fecha,
                id_cotizacion, id_version_cotizacion, id_usuario,
                atributo, valor_propuesto, valor_actual)
                
            VALUES ($this->id, '$fecha',
                '$id_cotizacion', '$id_version_cotizacion', $id_usuario,
                '$attrName', '$proposedValue', '$actualValue')";
        ...
        if($this->origen!="NO_IMPORTA"){
            if( intval($this->id_catalogo_productos)!=0){
                $consulta  = "SELECT factor, factor2 FROM CatalogoProductos
                            WHERE id_proveedor = ".$this->id_proveedor." AND id = ".$this->id_catalogo_productos;
                ...
                $result = mysql_query($query) or print("Error actualizando el precio y origen del producto");
    ...
    /*
     * METODO ELIMINADO POR LOS METODOS
     * uploadImage($campo)
     * uploadFichaTecnica($campo)
     */
//    public function updateFromVistaLectura($archivo_ficha_tecnica="",$imagen="",
//            $descripcion="",$unidad_metrica="",$unidad_metrica_ingles=""){
//        
//            $campo = 'imagen';
