...
/*$consulta="select industria from Clientes where id=$id_cliente";
$resultado = mysql_query($consulta) or print("La consulta fallo lista clientes: $consulta" . mysql_error());
$res=mysql_fetch_assoc($resultado);
$industria=$res['industria'];

if($industria==1)
{
	$buscar="where industria=1";//mineria
}
else
{
	$buscar="where industria<>1";
}*/

//print_r($_REQUEST);
if($_GET['id']!=""){
    $contacto = new Contacto();
    $contacto -> get($_GET['id'], $id_cliente );
    $_SESSION['cambiaContacto'] = $contacto;
    if($_GET['borrar']=='true'){
        $contacto->delete();
        unset($_SESSION['cambiaContacto']);
        ?><script>parent.location.reload();</script><?
    }
} else if( isset($_SESSION['cambiaContacto']) && $_GET['nuevo']!='true' ){
    $contacto = $_SESSION['cambiaContacto'];
} else {
    unset($_SESSION['cambiaContacto']);
    $contacto="";
}
$idcon=$contacto->id;
$consulta="select departamento_empresa AS departamento from Contactos where id=$idcon";
$resultado = mysql_query($consulta) or print("La consulta fallo lista clontactos: $consulta" . mysql_error());
$res=mysql_fetch_assoc($resultado);
$departamento=$res['departamento'];
echo "<script>alert('$departamento');</script>";
$consultad="select industria from departamentos where nombre='$departamento'";
$resultadod = mysql_query($consultad) or print("La consulta fallo departamentos: $consultad" . mysql_error());
$resd=mysql_fetch_assoc($resultadod);
$industria=$resd['industria'];
echo "<script>alert('$industria');</script>";
...
{
	$buscar="where industria=0";
}
else
{
	$buscar="where industria>1 and industria<=3";
}
...
if($_POST["guardar"]=="Guardar"){
    ...
    if($contacto->update($_SESSION['cambiaContacto']->id, $id_cliente,
            $nombre_contacto, $email_contacto, $telefono_contacto,
            $contrasenia, $departamento_empresa, $es_comprador)){
    ...
    <tr>
	  ...
	  <table width="100%" border="0" cellpadding="1" cellspacing="0" bordercolor="#CCCCCC">
        ...
        <tr>
          ...
          <td class="style5"><!--<input name="departamento_empresa" type="text" class="texto_info_negro" id="departamento_empresa" value="<?//php echo $contacto->departamento_empresa; ?>" size="45" maxlength="100" />-->
		  <select name="departamento_empresa" id="departamento_empresa" style="width:200px">
            <option value="">--Departamento--</option>
            <?php
	    $consulta87  = "SELECT * FROM departamentos ".$buscar." ORDER BY nombre ASC";
   ...
   <table width="450" border="0" align="center">
    <tr>
      <td class="style8"><div align="center">
        <input name="<? if($contacto != "") echo "guardar"; else echo "crear";?>" type="submit" class="texto_info" 
