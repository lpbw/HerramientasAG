...
setAccesoIndividual($_SESSION['accesos']['vendedor'],
...
$vistaLectura = tienePermisoEscritura(
        $_SESSION['accesos']['mostrador'],
        $_SESSION['accesos']['vendedor'],
        $_SESSION['accesos']['vendedor25']);

$editarProvYCatal = tienePermisoEscritura(
        $_SESSION['accesos']['administrador'],
        $_SESSION['accesos']['supervisor'],
        $_SESSION['accesos']['compras']);
...
//            UPDATE CODIGO 
            $updateSuccess4 = $producto->updateOneAttr($attrName='codigo', $attrValue=$_POST['codigo'], 
                    $_SESSION['usuario']->id, $requireRevision=FALSE,0,0);
//            UPDATE CODIGO INTERNO
            $codigo_interno = $producto->constructCodigoInterno();
            $updateSuccess5 = $producto->updateOneAttr($attrName='codigo_interno', $codigo_interno, 
                    $_SESSION['usuario']->id, $requireRevision=FALSE,0,0);
            
            $updateSuccess = $updateSuccess1 && $updateSuccess2 && $updateSuccess3 
                        && $updateSuccess4  && $updateSuccess5 && $uploadImage && $uploadFichaTecnica ;
        }
        
        if( $updateSuccess ){
            if($_POST['guardar']=="Cambiar Origen"){
                ?><script>document.location = 'seleccionar_origen_producto.php?id=<?echo $producto->id;?>';</script><?
            } else {
                unset($_SESSION['cambiaProducto']);
                ...
                if($_REQUEST['from']=="adm_revision_cambios_productos.php"){ ?>
                    <script>
                        parent.document.getElementById('no_aprobar_<? echo $producto->id.$_REQUEST['atributo']?>').checked = true;
                        parent.cerrarV();
                    </script> <?
                } else if( $pos === false ){
                    ?><script>parent.location.reload();</script><?
            ...
            ?><script>alert('Verifica tu producto, algunos datos no se cargaron correctamente');</script><?
        }
}
...
//            NORMAL UPDATE
            $updateSuccess3 = $producto -> update($nombre,'id',$id_catalogo_productos,
                    $precio,$costo,$id_proveedor,$modificado,$_FILES['archivo_ficha_tecnica']['name'],
                    $_FILES['imagen']['name'],$descripcion, $codigo_familia,$unidad_metrica, 
                    $exportar_microsip="", $stock, $codigo_descuento,
                    $tipo_moneda_usa,$unidad_metrica_ingles,$recargo=0,$esProductoEspecial = FALSE);
    ...
    function validar(){
        ...
        $( "select" ).each(function( index ) {
            ...
            if( ($(this).val() == "" || $(this).val() == undefined ||
                    $(this).val() == 0 || $(this).val() == "0" ) && $(this).attr('name')!= 'codigo_familia' && $(this).attr('name')!= 'entrega'){
                $(this).focus();
...
<div style="height: 453px">
  ...
  <table width="695" border="0" align="center" cellpadding="2" cellspacing="2" bgcolor="#FFFFFF">
    <tr>
      <td colspan="3"><div align="center" class="texto_info_blanco" style="background-image: url(images/bkg_1.jpg);">PRODUCTO
    ...
    <tr>
      <td width="127" align="right" valign="top" bgcolor="#E3E3E3" class="texto_info_negro">Nombre Espa&ntilde;ol</td>
      <td width="219" bordercolor="#CCCCCC" class="texto_info_negro"><? if($vistaLectura) { 
        ...
        <? 	}?></td>
      <td width="284" rowspan="13" class="texto_info_negro"><table border="0" cellspacing="0" cellpadding="1" bordercolor="#CCCCCC">
        <tr>
          <td>Imagen</td>
        ...
        <tr>
          <td colspan="2"><div align="center" id="imagen">
            <? if($producto->imagen!=""){?>
            <a href="<? echo $producto->imagen?>" target="_blank"><img src="<?php echo $producto->imagen;?>" alt="" name="imagenMostrar" height="150" id="imagenMostrar"></a>
