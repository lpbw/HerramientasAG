...
if($guardar == "Guardar"){
    ...
    if($producto -> update($nombre,'id',
        ...
        if( $pos === false ){
            ...
            ?><script>parent.location = 'generar_cotizacion_p.php?reloadCarritoOnId=<? echo $producto->id;?>';</script><?
        }
    }
}
...
//        if($_SESSION['cotizacion']->update( $_SESSION['cotizacion'] ) ){
        ?><script>window.parent.location = 'agregar_carrito.php?id=<?echo $producto->id;?>&cantidad=1&backTo=generar_cotizacion_p.php';</script><?
            ?><script>//parent.location.reload();</script><?
//        }
    }
}
...
function agregarCarrito($producto){
    ...
    if(isset($_SESSION['cotizacion'])){
        ...
        $_SESSION['cotizacion']->iva = $subtotal * 0.16;
        $_SESSION['cotizacion']->total = $subtotal * 1.16;
